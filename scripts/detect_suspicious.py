...
Seules les value bets sont alertées : une cote est comparée à la
probabilité "juste" dévigorée de Pinnacle (référence marché sharp), sans
IA, uniquement du calcul sur les cotes elles-mêmes (voir suspect.py). Les
alertes "match suspect" basées sur les mouvements de cotes dans le temps
ont été retirées de ce pipeline (jugées trop bruyantes/peu cohérentes) ;
le code reste dans suspect.py si besoin de les réactiver un jour.
...
from suspect import ValueBet, detect_value_bets
...
def _check_football(
    ...
    for event in events:
        _process_market(
            event, "h2h", "Cotes 1X2", "h2h", snapshots, value_alerted,
...
def _check_tennis(
    ...
    _process_market(
        match, "h2h", "Vainqueur du match", "tennis-h2h", snapshots, value_alerted,
...
def _send_summary(n_football: int, n_tennis: int, session_alerts: list) -> None:
    ...
    if session_alerts:
        text = (
            ...
            f"{tennis_line}"
            f"💎 {len(session_alerts)} value bet(s) détectée(s) (détails ci-dessus) :\n"
...
def main() -> None:
    ...
    n_football = _check_football(
        snapshots, value_alerted, current_state_keys, pending_results, session_alerts
    )
